...
public:
    __aicore__ inline void Init(GM_ADDR inputList, GM_ADDR output, const ConcatTilingData &t)
    {
        ...
        axis_ = t.axis;
        splitSegment_ = t.splitSegment;
        segmentChunkCount_ = t.segmentChunkCount;
    ...
    __aicore__ inline void Process(const ConcatTilingData &t)
    {
        ...
        for (uint64_t task = GetBlockIdx(); task < taskCount; task += blockDim_) {
            ...
            const uint32_t segmentChunk = splitSegment_
                ? static_cast<uint32_t>(rowTask % segmentChunkCount_)
                : 0;
            const uint64_t firstOuter = splitSegment_
                ? rowTask / segmentChunkCount_
                : rowTask * rowsPerTask_;
            ...
            if (splitSegment_) {
                const uint64_t done = static_cast<uint64_t>(segmentChunk) * tileElems_;
                if (done >= count) {
                    continue;
                }
...
private:
    ...
    GM_ADDR inputList_;
    GlobalTensor<T> out_;
    uint32_t inputCount_, blockDim_, rowsPerTask_, rowTaskCount_, axis_;
    uint32_t splitSegment_, segmentChunkCount_, tileElems_;
