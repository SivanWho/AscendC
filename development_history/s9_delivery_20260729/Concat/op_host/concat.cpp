...
static ge::graphStatus TilingFunc(gert::TilingContext *context)
{
    ...
    const uint64_t maxSegmentBytes = maxAxisSize * inner * elemBytes;
    ...
    const bool splitSegment = maxSegmentBytes > tileBytes;
    const uint32_t segmentChunkCount = splitSegment
        ? static_cast<uint32_t>(CeilDiv(maxSegmentBytes, tileBytes))
        : 1;
    const uint32_t rowsPerTask = splitSegment
        ? 1
        : static_cast<uint32_t>(
              std::min<uint64_t>(4095, std::max<uint64_t>(
                  1, std::min(rowsForUb, rowsForParallel))));
    const uint32_t rowTaskCount = outer == 0
        ? 0
        : static_cast<uint32_t>(
              splitSegment ? outer * segmentChunkCount
                           : CeilDiv(outer, rowsPerTask));
    ...
    tiling.set_splitSegment(splitSegment ? 1 : 0);
    tiling.set_segmentChunkCount(segmentChunkCount);
