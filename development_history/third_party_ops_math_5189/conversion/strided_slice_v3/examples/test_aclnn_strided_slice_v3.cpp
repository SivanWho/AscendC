...
int main()
{
    ...
    std::vector<int64_t> outShape = {1, 2, 2};
    ...
    std::vector<int64_t> beginData = {0, 0, 0};
    std::vector<int64_t> endData = {1, 2, 2};
    std::vector<int64_t> stridesData = {1, 1, 1};
    std::vector<int64_t> axesData = {0, 1, 2};
    std::vector<float> outHostData(4, 0.0);
    ...
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建aclIntArray
    begin = aclCreateIntArray(beginData.data(), 3);
    CHECK_RET(begin != nullptr, return ret);
    end = aclCreateIntArray(endData.data(), 3);
    CHECK_RET(end != nullptr, return ret);
    axes = aclCreateIntArray(axesData.data(), 3);
    CHECK_RET(axes != nullptr, return ret);
    strides = aclCreateIntArray(stridesData.data(), 3);
    ...
    // 调用aclnnCast第一段接口
    ret = aclnnSliceV2GetWorkspaceSize(
        self, begin, end, axes, strides, out, &workspaceSize,
        &executor);
