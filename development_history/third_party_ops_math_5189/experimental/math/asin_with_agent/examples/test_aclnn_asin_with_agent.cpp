...
#include <cmath>
#include "acl/acl.h"
#include "../op_api/aclnn_asin_with_agent.h"
...
int main()
{
    ...
    ret = aclnnAsinWithAgentGetWorkspaceSize(self, out, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnAsinWithAgentGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    ...
    // 4、调用asinh算子
    ret = aclnnAsinWithAgent(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnAsinWithAgent failed. ERROR: %d\n", ret); return ret);
