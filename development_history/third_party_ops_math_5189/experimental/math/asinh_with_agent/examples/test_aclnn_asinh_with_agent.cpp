...
#include <cmath>
#include "acl/acl.h"
#include "../op_api/aclnn_asinh_with_agent.h"
...
int main()
{
    ...
    ret = aclnnAsinhWithAgentGetWorkspaceSize(self, out, &workspaceSize, &executor);
    CHECK_RET(
        ret == ACL_SUCCESS, LOG_PRINT("aclnnAsinhWithAgentGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    ...
    // 4、调用asinh算子
    ret = aclnnAsinhWithAgent(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnAsinhWithAgent failed. ERROR: %d\n", ret); return ret);
