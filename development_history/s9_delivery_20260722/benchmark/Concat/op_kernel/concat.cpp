    ...
    __aicore__ inline void Process(const ConcatTilingData &t)
    {
        ...
        for (uint64_t task = GetBlockIdx(); task < taskCount; task += blockDim_) {
            ...
            for (uint64_t done = 0; done < count; done += tileElems_) {
                ...
                event_t inputReady = static_cast<event_t>(
                ...
                event_t outputDone = static_cast<event_t>(
                    GetTPipePtr()->FetchEventID(HardEvent::MTE3_MTE2));
                SetFlag<HardEvent::MTE3_MTE2>(outputDone);
                WaitFlag<HardEvent::MTE3_MTE2>(outputDone);
            }
        }
    }
