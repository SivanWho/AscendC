...
static ge::graphStatus TilingFunc(gert::TilingContext *ctx)
{
    ...
    // V1 correctness baseline: int8/bool SetValue writes may update a wider GM
    // transaction. Strided writes from multiple cores can therefore overwrite
    // neighboring bytes. Keep one core until V2 introduces 32-byte-aligned,
    // contiguous per-core output partitions.
...
public:
    explicit Greater(const char *name) : OpDef(name)
    {
        ...
        config.DynamicCompileStaticFlag(true)
            ...
            .ExtendCfgInfo("opFile.value", "greater");
