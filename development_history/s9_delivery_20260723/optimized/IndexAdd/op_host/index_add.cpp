...
namespace {
constexpr uint64_t kTileElements = 8192;
...
static ge::graphStatus TilingFunc(gert::TilingContext *ctx)
{
    ...
    const uint64_t selfCount = s.GetShapeSize();
    const uint64_t selfDim = s.GetDim(dim);
    const uint64_t denominator = selfDim * inner;
    const uint64_t outer = denominator == 0 ? 0 : selfCount / denominator;
    const uint64_t chunksPerRow =
        inner == 0 ? 0 : (inner + kTileElements - 1) / kTileElements;
    const uint64_t taskCount = outer * selfDim * chunksPerRow;
    const uint32_t blockDim = static_cast<uint32_t>(
        taskCount < kVectorCoreCount ? taskCount : kVectorCoreCount);
    ctx->SetBlockDim(blockDim == 0 ? 1 : blockDim);
