...
install_root="$root/installs_final"
test_root="$root/tests_final"
if [[ -e "$install_root" || -e "$test_root" ]]; then
  echo 'Refusing to overwrite existing final smoke-test state' >&2
  exit 1
fi
...
for op in Concat Greater SquareSumV1; do
  ...
  python3 setup.py build bdist_wheel > "$root/final_extension_${op}.log" 2>&1
  python3 -m pip install --force-reinstall --no-deps dist/custom_ops*.whl \
    > "$root/final_extension_install_${op}.log" 2>&1
  ...
  for variant in benchmark optimized; do
    ...
    echo "===== INSTALL $key"
    "$run" --quiet --install-path="$install" > "$root/final_install_${key}.log" 2>&1
    ...
    echo "===== TEST $key"
    timeout 180 python3 test_op.py 1 2>&1 | tee "$root/final_test_${key}.log"
    grep -q 'verify result pass' "$root/final_test_${key}.log"
  done
done

echo '===== ALL SIX FINAL PUBLIC CASE RUNS PASSED'
