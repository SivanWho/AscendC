...
class KernelConcat {
public:
    __aicore__ inline void Init(GM_ADDR inputList, GM_ADDR output,
...
extern "C" __global__ __aicore__ void concat(GM_ADDR inputs, GM_ADDR y,
                                               GM_ADDR workspace, GM_ADDR tiling) {
    GET_TILING_DATA(tilingData, tiling);
    if (tilingData.dtypeCode == 0) RunConcat<half>(inputs, y, tilingData);
    else if (tilingData.dtypeCode == 1) RunConcat<float>(inputs, y, tilingData);
    else if (tilingData.dtypeCode == 2) RunConcat<int32_t>(inputs, y, tilingData);
    else RunConcat<int8_t>(inputs, y, tilingData);
}
