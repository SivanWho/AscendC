...
static ge::graphStatus TilingFunc(gert::TilingContext *context) {
    ...
    if (inputCount == 0 || inputCount > CONCAT_MAX_INPUTS) return ge::GRAPH_FAILED;
    ...
    if (dim < 0) dim += rank;
    if (rank <= 0 || dim < 0 || dim >= rank) return ge::GRAPH_FAILED;

    uint64_t outer = 1, inner = 1;
    for (int32_t i = 0; i < dim; ++i) outer *= static_cast<uint64_t>(shape.GetDim(i));
    for (int32_t i = static_cast<int32_t>(dim) + 1; i < rank; ++i) inner *= static_cast<uint64_t>(shape.GetDim(i));
    ...
    for (uint32_t i = 0; i < inputCount; ++i) {
        ...
        if (cur.GetDimNum() != static_cast<size_t>(rank)) return ge::GRAPH_FAILED;
...
static graphStatus InferShape(gert::InferShapeContext *context) {
    ...
    if (attrs == nullptr || first == nullptr || attrs->GetInt(0) == nullptr) return GRAPH_FAILED;
    ...
    *out = *first;
    ...
    for (size_t i = 0; i < count; ++i) sum += context->GetDynamicInputShape(0, i)->GetDim(dim);
    out->SetDim(dim, sum);
