...
uint32_t DtypeCode(ge::DataType dt) {
    if (dt == ge::DT_FLOAT16) return 0;
    if (dt == ge::DT_FLOAT) return 1;
    if (dt == ge::DT_INT32) return 2;
    if (dt == ge::DT_INT8) return 3;
    return 4;
}
}
namespace optiling {
static ge::graphStatus TilingFunc(gert::TilingContext *ctx) {
    ...
    if (rank == 0 || rank > 8) return ge::GRAPH_FAILED;
    uint64_t out[8] = {}, sa[8] = {}, sb[8] = {};
    uint64_t aRaw[8] = {}, bRaw[8] = {};
    uint64_t stride = 1;
    for (int i = static_cast<int>(a.GetDimNum()) - 1; i >= 0; --i) { aRaw[i] = stride; stride *= a.GetDim(i); }
    stride = 1;
    for (int i = static_cast<int>(b.GetDimNum()) - 1; i >= 0; --i) { bRaw[i] = stride; stride *= b.GetDim(i); }
    uint64_t total = 1;
    for (uint32_t d = 0; d < rank; ++d) {
        ...
        if (ad != bd && ad != 1 && bd != 1) return ge::GRAPH_FAILED;
        out[d] = std::max(ad, bd);
    ...
    const uint32_t blocks = std::max<uint32_t>(1, std::min<uint64_t>(platform.GetCoreNum(), total));
    GreaterTilingData t;
    t.set_dtypeCode(DtypeCode(ctx->GetInputDesc(0)->GetDataType()));
    t.set_rank(rank); t.set_blockDim(blocks); t.set_outputCount(total);
    t.set_outputShape(out); t.set_x1Strides(sa); t.set_x2Strides(sb);
    ...
    ctx->SetBlockDim(blocks); ctx->GetWorkspaceSizes(1)[0] = 0;
    return ge::GRAPH_SUCCESS;
}
}
namespace ge {
static graphStatus InferShape(gert::InferShapeContext *ctx) {
    const auto *a = ctx->GetInputShape(0); const auto *b = ctx->GetInputShape(1);
...
static graphStatus InferType(gert::InferDataTypeContext *ctx) { ctx->SetOutputDataType(0, ge::DT_BOOL); return GRAPH_SUCCESS; }
}
namespace ops {
class Greater : public OpDef {
public:
 explicit Greater(const char *name) : OpDef(name) {
    const std::initializer_list<ge::DataType> in = {ge::DT_FLOAT, ge::DT_BF16, ge::DT_FLOAT16, ge::DT_INT32, ge::DT_INT8};
    const std::initializer_list<ge::Format> fmt = {ge::FORMAT_ND, ge::FORMAT_ND, ge::FORMAT_ND, ge::FORMAT_ND, ge::FORMAT_ND};
    this->Input("self").ParamType(REQUIRED).DataType(in).Format(fmt);
    this->Input("other").ParamType(REQUIRED).DataType(in).Format(fmt);
    this->Output("y").ParamType(REQUIRED).DataType({ge::DT_BOOL, ge::DT_BOOL, ge::DT_BOOL, ge::DT_BOOL, ge::DT_BOOL}).Format(fmt);
