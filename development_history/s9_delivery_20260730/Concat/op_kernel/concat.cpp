...
public:
    ...
    __aicore__ inline void Process(const ConcatTilingData &t)
    {
        ...
        uint64_t cursorRowTask = UINT64_MAX;
        uint64_t cursorPrefix = 0;
        uint32_t cursorNextInput = 0;
...
private:
    __aicore__ inline void GetAxisInfo(
        ...
        uint32_t inputId,
        uint64_t rowTask,
        uint64_t &cursorRowTask,
        uint32_t &cursorNextInput,
        uint64_t &cursorPrefix,
        uint64_t &axisSize,
        uint64_t &axisPrefix)
    {
        ...
        if (cursorRowTask != rowTask) {
            cursorRowTask = rowTask;
            cursorNextInput = 0;
            cursorPrefix = 0;
        }
        ...
        while (cursorNextInput <= inputId) {
            list.GetDesc(desc, cursorNextInput);
            const uint64_t currentAxisSize = desc.GetShape(axis_);
            if (cursorNextInput == inputId) {
                axisPrefix = cursorPrefix;
                axisSize = currentAxisSize;
            }
            cursorPrefix += currentAxisSize;
            ++cursorNextInput;
        }
    }
...
template <typename T>
__aicore__ inline void RunConcat(
    GM_ADDR inputs,
    GM_ADDR y,
    const ConcatTilingData &t)
{
