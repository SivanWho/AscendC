...
static ge::graphStatus TilingFunc(gert::TilingContext *context)
{
    ...
    // Above the inline metadata limit, each active core must walk the dynamic
    // descriptor list once per row task.  Do not start more cores than the
    // payload size can usefully occupy, otherwise scalar descriptor traffic
    // grows with no corresponding DMA throughput benefit.
    const uint64_t coreLimit =
        inputCount > CONCAT_MAX_INPUTS ? targetCores : availableCores;
    const uint32_t blockDim =
        std::max<uint32_t>(1, std::min<uint64_t>(coreLimit, tasks));
