...
static ge::graphStatus TilingFunc(gert::TilingContext *ctx)
{
    ...
    const uint64_t lastDim = shape[rank - 1];
    const uint64_t paddedLastDim = (lastDim + 15) / 16 * 16;
    const bool lastAxisOnly = axes->GetSize() == 1 && reduced[rank - 1] == 1;
    const uint32_t fastPath = Code(ctx->GetInputDesc(0)->GetDataType()) == 0 && lastAxisOnly &&
                              paddedLastDim <= 64 && outCount <= 255 && outCount * paddedLastDim <= 32768;
    uint32_t blocks = 1;
    SquareSumV1TilingData t;
    t.set_dtypeCode(Code(ctx->GetInputDesc(0)->GetDataType()));
    t.set_rank(rank);
    t.set_axisCount(axes->GetSize());
...
static graphStatus InferShape(gert::InferShapeContext *ctx)
{
    ...
    bool reduced[8] = {};
    for (size_t i = 0; i < a->GetListInt(0)->GetSize(); ++i) {
        int64_t d = a->GetListInt(0)->GetData()[i];
        if (d < 0) {
            d += x->GetDimNum();
        }
        reduced[d] = true;
    ...
    size_t outRank = keep ? x->GetDimNum() : x->GetDimNum() - a->GetListInt(0)->GetSize();
    y->SetDimNum(outRank);
    size_t o = 0;
    for (size_t d = 0; d < x->GetDimNum(); ++d) {
        if (reduced[d]) {
            if (keep) {
                y->SetDim(o++, 1);
            }
