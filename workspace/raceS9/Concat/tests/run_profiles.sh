...
install_root=$2
shift 2
# The official extension already launches 30 timed kernels per custom_op call.
rounds=${PROFILE_ROUNDS:-1}
...
for case_name in "$@"; do
    ...
    msprof \
        --output="${output_dir}" \
        --application="python3 concat_profile_matrix.py ${case_name} --rounds ${rounds}" \
